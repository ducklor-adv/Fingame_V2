...
(function runDevTests() {
  try {
    ...
    console.assert(
      /^[0-9]{2}[A-Z]{3}[0-9]{4}$/.test(id1) && /^[0-9]{2}[A-Z]{3}[0-9]{4}$/.test(id2),
      "UserId format must be YYAAA####"
    );
    ...
    const acfRootActual = usersBase.some((u) => u.id === acfRootWanted)
      ? acfRootWanted
      : SYSTEM_ROOT_ID;
    console.assert(
      acfRootActual === SYSTEM_ROOT_ID,
      "Fallback to system root if default ACF root not present"
    );

    const depthMap = relativeDepthMap(usersBase, acfRootActual);
    const nicScoped = usersBase.filter(
      (u) => bfsSubtree(usersBase, acfRootActual).has(u.id) && u.id !== SYSTEM_ROOT_ID
    );
    const nicSorted = nicScoped.sort(cmpNICWithDepth(depthMap)).map((u) => u.id);
    console.assert(
      nicSorted[0] === A.id || nicSorted[0] === B.id,
      "[NIC BFS] shallower under ACF root first"
    );
    ...
    console.assert(
      bicSorted[0] === B.id,
      "[BIC BFS] inviter at depth 0 first if not full"
    );
  } catch (e) {
    // eslint-disable-next-line no-console
    console.warn("[ACF DevTests] Warning:", e);
    // If DevTests fail, it should not break the UI; only log to console.
  }
})();
