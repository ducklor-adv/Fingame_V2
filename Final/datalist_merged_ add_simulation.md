...
-----------------------------
## 18. simulated_fp_transactions

```sql
CREATE TABLE simulated_fp_transactions (
  id TEXT PRIMARY KEY,                       -- Simulation transaction ID (UUID string)

  -- Owner of this simulation event
  user_id TEXT NOT NULL,                     -- FK -> users(id), เจ้าของเหตุการณ์/ต้นทาง

  -- Business meaning
  simulated_tx_type TEXT NOT NULL,           -- SECONDHAND_SALE, EXP_PRB, EXP_CAR, EXP_HEALTH
  simulated_source_type TEXT NOT NULL,       -- SIM_SECONDHAND_SALE, SIM_EXP_TRANSACTION, etc.
  simulated_source_id TEXT,                  -- อ้างอิงถึงสินค้าหรือดีลประกัน (ถ้ามี)

  -- Base numbers used to calculate FP
  simulated_base_amount REAL NOT NULL,       -- ยอดเงินตั้งต้น (THB) เช่น ยอดขายของมือสอง / ค่าเบี้ย
  simulated_reverse_rate REAL NOT NULL,      -- อัตรา reverse FP เช่น 0.07

  -- FP generated and allocation breakdown
  simulated_generated_fp REAL NOT NULL,      -- FP ทั้งก้อนที่เกิดจากเหตุการณ์นี้ (ก่อนแบ่ง)
  simulated_system_cut_rate REAL DEFAULT 0.10,
  simulated_system_cut_fp REAL DEFAULT 0.0,
  simulated_self_rate REAL DEFAULT 0.45,
  simulated_self_fp REAL DEFAULT 0.0,
  simulated_network_rate REAL DEFAULT 0.45,
  simulated_network_fp REAL DEFAULT 0.0,
  simulated_upline_depth INTEGER DEFAULT 7,  -- จำนวนชั้นสูงสุดที่ใช้กระจาย (รวมตัวเอง)

  -- Status of posting to ledger
  simulated_status TEXT NOT NULL DEFAULT 'PENDING', -- PENDING, COMPLETED, FAILED
  simulated_error_message TEXT,
  simulated_run_mode TEXT DEFAULT 'AUTO',    -- AUTO / MANUAL (สำหรับอนาคต)
...
-----------------------------
## 19. simulated_fp_ledger

```sql
CREATE TABLE simulated_fp_ledger (
  id TEXT PRIMARY KEY,                       -- Ledger line ID (UUID string)

  -- Link to simulation transaction
  simulated_tx_id TEXT NOT NULL,             -- FK -> simulated_fp_transactions(id)

  -- Who is affected by this FP line
  user_id TEXT NOT NULL,                     -- FK -> users(id)
  related_user_id TEXT,                      -- ใครเป็นต้นทางของ FP บรรทัดนี้ (เช่น เจ้าของรายการขาย)

  -- Network / role context
  level INTEGER,                             -- 0 = ตัวเอง, 1..6 = upline ตามลำดับ
  simulated_entry_role TEXT,                 -- ORIGIN_SELF, UPLINE_BONUS, SYSTEM_FEE, EXP_DISCOUNT

  -- Accounting style DR/CR
  dr_cr TEXT NOT NULL,                       -- 'DR' หรือ 'CR'
  simulated_fp_amount REAL NOT NULL,         -- จำนวน FP ที่รับ/ถูกหักในบรรทัดนี้
  simulated_balance_after REAL,              -- ยอด FP คงเหลือของ user นี้หลังบรรทัดนี้

  -- Copy of business context for easier querying
  simulated_tx_type TEXT NOT NULL,           -- SECONDHAND_SALE, EXP_PRB, EXP_CAR, EXP_HEALTH
  simulated_source_type TEXT,
  simulated_source_id TEXT,

  -- Date parts for reporting (แยกวัน เดือน ปี ตามที่คุณต้องการ)
  simulated_tx_datetime TEXT,                -- ISO timestamp ของรายการนี้
